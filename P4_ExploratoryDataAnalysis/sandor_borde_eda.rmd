...
head(ratio_by_city[order(-ratio_by_city$sum), c("contbr_city", "sum", "ratio")], n = 10)
```
...
other, but not in all cases. The number and sum of all support what each 
candidate received correlated strongly with each other (r = 0.94).
...
largest amount came. These two city has almost 20% of all contributions.
